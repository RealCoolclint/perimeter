...
*Version 1.19 — 20 septembre 2026*
...
### Direction visuelle (D24, remplacé intégralement par D79 ; encodage visuel amendé par D80 ; thème sombre étendu par D81)
...
Prototype de travail : maquettes interactives construites dans un canvas de design séparé (deux options comparées — index numéroté au survol, retenue ; labels rayonnants, écartée) avant la piste Arrival, puis reconstruites après son abandon. Portage vers le prototype réel du repo (`perimeter_parcours_prototype_v3.html`) **pas encore fait** — voir section 11.
...
- **Brancher Outlook mail au pipeline (D48)** dès que le consentement admin est accordé (D74, bloqué) — reconnecter le credential, retester isolé avant de construire la boucle dans "Préparer signaux Ollama", sur le même modèle que la boucle Gmail (10nonies).
...
- **Chantier design du cercle à 15 azimuts (D53), suite** — primitif Cercle + Point validé (D80), aligné sur le logo Perimeter réel. Reste à faire : porter la version validée dans le prototype réel `perimeter_parcours_prototype_v3.html` via prompt Cursor ; verser le logo Perimeter (2 variantes fournies par Martin) dans le repo, à un emplacement à définir ; construire une maquette du thème sombre (D81) ; décider du sort de l'Option B ("labels rayonnants"), non mise à jour depuis l'abandon de sa comparaison avec l'Option A.
