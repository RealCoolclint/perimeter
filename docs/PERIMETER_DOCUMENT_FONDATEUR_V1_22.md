...
*Version 1.22 — 26 septembre 2026 (soir)*
...
- **D89** — Keep-alive Supabase par **GitHub Actions**, dans le repo `perimeter-site` (`.github/workflows/supabase-keepalive.yml`) : une lecture minimale quotidienne (`GET /rest/v1/points?select=id&limit=1`, 05:17 UTC) pour éviter la mise en pause du free tier après 7 jours d'inactivité. Choisi plutôt qu'un workflow n8n, qui ne tournerait que Mac Maison allumé et Docker lancé. Authentification par la seule clé publique `anon` (secret GitHub `SUPABASE_ANON_KEY`), jamais la clé `service_role`. Premier run manuel validé le 26/09/2026 (succès, 12 s). Limite connue : GitHub suspend les tâches planifiées d'un repo public sans activité pendant 60 jours (e-mail d'avertissement, réactivation en un clic dans l'onglet Actions).
